...
1.	Log in to the Rafay console. New Rafay customers will receive an email with a username, password, and link to the Rafay console.
2.	Navigate to `Infrastructure` and then `Clusters` to find details about the newly imported cluster, as shown in the following figure.
...
For best practices and general documentation, see https://docs.rafay.co/[Rafay's documentation^].
...
A third-party service is used for emails sent by the controller (for example, for first-time user activation, password resets, and notifications). To help guarantee delivery of emails from the SaaS controller to users in your organization, we recommend using an IP allow list for for inbound traffic. The dedicated IP address used for sending emails from the Rafay Controller is `149.72.39.92`.

=== Network firewall rules

The Rafay Controller is designed for customers to deploy and manage clusters in both public and on-premises environments. To onboard an on-premise or cloud-based cluster onto for ongoing operations and lifecycle management, ensure the following:
